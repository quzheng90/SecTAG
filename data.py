...
#
def read_hijack_file(filepath:str):

    df = pd.read_csv(filepath, sep=',', keep_default_na=False)
    df=df[:]
    content_ = np.array(df['s2'].values)

    # Process tweets
    hashtag_= np.array(df['s1'].values)
    ...
    return nums, hashtag_, content_, label_a, label_b, label_c

def read_test_file(task, tokenizer, truncate=512):
    df1 = pd.read_csv(os.path.join(HASH_PATH, 'testset-level' + task + '.tsv'), sep='\t')
    df2 = pd.read_csv(os.path.join(HASH_PATH, 'labels-level' + task + '.csv'), sep=',')
...
###
def read_test_file_hijack(filepath:str, tokenizer,truncate=512):
    ...
    hashtag_= np.array(df['s1'].values)
    nums = len(df)
    token_ids = [tokenizer.encode(text=content_[i], add_special_tokens=True, max_length=truncate) for i in range(nums)]
    mask = np.array(get_mask(token_ids))
    lens = get_lens(token_ids)
    token_ids = np.array(pad_sents(token_ids, tokenizer.pad_token_id))
    label_a = np.array(df['subtask_a'].values)
    ...
    return token_ids, lens, mask, label_a, label_b, label_c

def read_test_file_all(tokenizer, truncate=512):
    df = pd.read_csv(os.path.join(HASH_PATH, 'testset-levela.tsv'), sep='\t')
    df_a = pd.read_csv(os.path.join(HASH_PATH, 'labels-levela.csv'), sep=',')
    ids = np.array(df['id'].values)
    tweets = np.array(df['tweet'].values)   
    label_a = np.array(df_a.iloc[:,1].values)
    ...
    df_b = pd.read_csv(os.path.join(HASH_PATH, 'labels-levelb.csv'), sep=',')
    df_c = pd.read_csv(os.path.join(HASH_PATH, 'labels-levelc.csv'), sep=',')
    label_data_b = dict(zip(df_b.iloc[:,0].values,df_b.iloc[:,1].values))
    label_data_c = dict(zip(df_c.iloc[:,0].values, df_c.iloc[:,1].values))
...
#针对hijack任务的构建模式
def all_task_hijack(filepath: str, tokenizer, truncate=512):

    nums,hashtag_, content_, label_a, label_b,label_c=read_hijack_file(filepath)
